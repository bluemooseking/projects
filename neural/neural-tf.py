...
X = np.array(df.loc[0:, 0:0], dtype=np.float64)
y = np.array(df.loc[0:, 3:3], dtype=np.float64)
...
def base_merge(X, base):
    X_out = np.zeros([X.shape[0], 1])
    
    while (1):
        X_out = np.add(X_out, X[:, -1].reshape([-1,1]))
        X = np.delete(X, -1, axis=1)
        if (X.shape[1] == 0):
            break
        X_out = np.multiply(X_out, base)
        
    return X_out

X_splice = base_splice(X, splice_base_val)
y_splice = base_splice(y, splice_base_val)

# Create sets
X_train, X_garbage, y_train, y_garbage = train_test_split(X_splice, y_splice, test_size=0.995)
...
def initialize_parameters(n_x, dims, n_y):
    ...
    for l in range(1, L):
        sWx = 'W' + str(l)
        sbx = 'b' + str(l)
        parameters[sWx] = tf.get_variable(
                sWx,
                [dims[l], dims[l-1]],
                initializer = tf.contrib.layers.xavier_initializer(seed=None),
                dtype=tf.float64)
        parameters[sbx] = tf.get_variable(
                sbx, 
                [dims[l], 1], 
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs + 1):
            ...
            if print_cost == True and epoch % 100 == 0:
                print ("Cost after epoch %i: %e" % (epoch, epoch_cost))
            if print_cost == True:
...
def extend_data(X, factor, count):
    xmin = np.amin(X)
    xmax = np.amax(X)
    xmid = xmin + xmax / 2
    xprt = xmid * (1 - factor)
    return np.linspace(xprt + (xmin * factor), 
                       xprt + (xmax * factor), 
                       count,
...
y_pred_splice = predict(X_test, parameters)
y_pred_nom = (y_pred_splice > 0.5)

# X_ext = extend_data(X_test, 1.5, 100)
# y_ext = predict(X_ext, parameters)
X_test_merge = base_merge(X_test, splice_base_val)
y_test_merge = base_merge(y_test, splice_base_val)
y_pred_merge = base_merge(y_pred_nom, splice_base_val)

plt.plot(list(X_test_merge), list(y_test_merge), 'ro', markersize=4)
plt.plot(list(X_test_merge), list(y_pred_merge), 'bo', markersize=4)
# plt.plot(list(X_ext), list(y_ext), 'yo', markersize=2)
